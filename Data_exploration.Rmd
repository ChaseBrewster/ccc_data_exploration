...
###Updating the polymer percentage graph with new data (5/2023) + Jamaica + Panama
#Wrangling data to look at polymer breakdowns
```{r}
ccc_data_5_23 <- read_csv("CCC_MonthlyDataLog_Responses_5_23.csv")

ccc_data_5_23 <- head(ccc_data_5_23, 481) %>% 
  select(1:65)

summary(ccc_data_5_23)

ccc_data_5_23$AT <- as.numeric(ccc_data_5_23$AT)
ccc_data_5_23$AU <- as.numeric(ccc_data_5_23$AU)
ccc_data_5_23$AV <- as.numeric(ccc_data_5_23$AV)
ccc_data_5_23$AW <- as.numeric(ccc_data_5_23$AW)
ccc_data_5_23$AX <- as.numeric(ccc_data_5_23$AX)
ccc_data_5_23$AY <- as.numeric(ccc_data_5_23$AY)
ccc_data_5_23$AZ <- as.numeric(ccc_data_5_23$AZ)
ccc_data_5_23$BA <- as.numeric(ccc_data_5_23$BA)

ccc_data_5_23 <- ccc_data_5_23 %>% 
  rename(
    LDPE = AT,
    HDPE = AU,
    PP = AV,
...
#saving data
write_csv(ccc_data_5_23, file = "ccc_data_5_23.csv")
```
...
ccc_data_polymer_5_23 <- ccc_data_5_23 %>% 
  select(C, D, E, LDPE, HDPE, PP, PVC, PS, Nylon, Teflon, PET) %>% 
  mutate(total = LDPE + HDPE + PP + PVC + PS + Nylon + Teflon + PET) %>% 
  mutate(LDPE_p = (LDPE/total)*100) %>%
  mutate(HDPE_p = (HDPE/total)*100) %>%
...
ccc_data_polymer_5_23_short <- ccc_data_5_23 %>% 
  select(C, D, E, LDPE, HDPE, PP, PVC, PS, Nylon, Teflon, PET) %>% 
  mutate(total = LDPE + HDPE + PP + PVC + PS + Nylon + Teflon + PET)

polymer_summary_5_23 <- ccc_data_polymer_5_23_short %>% 
  group_by(E) %>% 
  summarise(
    LDPE = sum(LDPE, na.rm = TRUE),
    HDPE = sum(HDPE, na.rm = TRUE),
...
polymer_summary_5_23 <- polymer_summary_5_23[c(2, 4:10),]

polymer_summary_5_23_p <- polymer_summary_5_23 %>% 
  mutate(total = LDPE + HDPE + PP + PVC + PS + Nylon + Teflon + PET) %>% 
  mutate(LDPE_p = (LDPE/total)*100) %>%
  mutate(HDPE_p = (HDPE/total)*100) %>%
  mutate(PP_p = (PP/total)*100) %>%
...
polymer_pivot_5_23 <- polymer_summary_5_23 %>% 
  pivot_longer(
    cols = 2:9,
    names_to = "Polymer",
    values_to = "Total"
...
polymer_pivot_5_23 <- polymer_pivot_5_23 %>% 
  group_by(E)
...
polymer_pivot_5_23$Country <- factor(polymer_pivot_5_23$Country, levels = c("Kenya", "Ecuador", "Mexico", "Indonesia", "Thailand", "Vietnam", "Panama", "Jamaica"))

class(polymer_pivot_5_23$Polymer)

polymer_pivot_5_23$Polymer <- as.factor(polymer_pivot_5_23$Polymer)

polymer_pivot_5_23$Polymer <- factor(polymer_pivot_5_23$Polymer, levels = c("Nylon", "Teflon","PVC", "LDPE", "PS", "PP","HDPE","PET"))

cbPalette_two <- c("#CC79A7", "#F0E442", "#D55E00", "#E69F00", "#0072B2", "#999999", "#56B4E9", "#009E73")

polymer_graph_5_23 <- ggplot(polymer_pivot_5_23, aes(fill=Polymer, y = Total, x = Country, group = Total)) +
  geom_bar(position = "fill", stat = "identity", color = 'black', size = 0.2) +
  theme_classic() +
  scale_fill_manual(values=cbPalette_two) +
  ggtitle("Polymer Type (%) by Country\n") +
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  scale_y_continuous(labels = percent, expand = c(0,0)) +
  annotate("text", label = "37.5%", x = 2, y = .19, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "40.7%", x = 4, y = .20, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "34.2%", x = 1, y = .17, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "66.0%", x = 3, y = .33, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "34.3%", x = 5, y = .17, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "52.9%", x = 6, y = .26, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "26.4%", x = 2, y = .51, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "32.6%", x = 4, y = .57, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "19.0%", x = 1, y = .44, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "31.3%", x = 3, y = .80, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "26.7%", x = 5, y = .48, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "14.4%", x = 6, y = .60, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "18.1%", x = 2, y = .73, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "22.8%", x = 4, y = .85, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "19.0%", x = 1, y = .63, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "25.0%", x = 5, y = .74, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "11.4%", x = 6, y = .73, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "15.8%", x = 2, y = .90, color = 'white', size = 3, fontface = 'bold') +  
  annotate("text", label = "76.2%", x = 7, y = .38, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "23.8%", x = 7, y = .88, color = 'white', size = 3, fontface = 'bold') +
  annotate("text", label = "95.5%", x = 8, y = .48, color = 'white', size = 3, fontface = 'bold')


polymer_graph_5_23

ggsave("polymer_graph_5_23.png")
